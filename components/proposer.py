...
from augmented_captioner.image_quality import get_aug_caption
...
class Proposer:
    ...
    def captioning(self, dataset: List[Dict]):
        for item in dataset:
            ...
            original_caption = get_vlm_output(
                item["path"],
                self.args["captioner"]["prompt"],
                self.args["captioner"]["model"],
            )
            ...
            if self.args['augmented']:
                augmented_caption = get_aug_caption(
                    self.args['augmented_prompt'],
                    item["path"]
                )
                original_caption = original_caption + ". " + augmented_caption

            print(original_caption)
            item["caption"] = original_caption
